...
export default function HouseTile({ house }: { house: HouseDTO }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (h?.status !== "processing" && h?.status !== "refreshing" && h?.status !== "scoring") {
          router.refresh();
        } else {
          setTimeout(poll, 800);
        }
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-cream-200 bg-white shadow-sm transition-shadow hover:shadow-md">
      <Link href={`/houses/${house.id}`} className="relative block aspect-[4/3] w-full overflow-hidden rounded-t-2xl bg-cream-100">
        {house.imagePath ? (
