...
import SchoolsForm from "@/client/components/SchoolsForm";
import type { CriteriaDTO, SchoolDTO } from "@/lib/types";
...
export function CriteriaPage() {
  ...
  const { data: schoolsData, isLoading: schoolsLoading } = useQuery({
    queryKey: ["schools"],
    queryFn: () => api.get<{ schools: SchoolDTO[] }>("/api/schools"),
  });
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-4 py-8">
      ...
      <div className="my-10 border-t border-cream-200" />

      <h2 className="mb-2 font-display text-2xl font-semibold text-brand-900">
        Basisscholen
      </h2>
      <p className="mb-6 text-sm text-brand-700">
        Geef de basisscholen op waartegen de afstand gemeten moet worden. Zo kan
        de afstand tot een basisschool betrouwbaar worden berekend.
      </p>
      {schoolsLoading ? (
        <div className="flex items-center justify-center py-12">
          <svg className="h-8 w-8 animate-spin text-brand-500" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        <SchoolsForm initial={schoolsData?.schools ?? []} />
      )}
    </div>
  );
}
